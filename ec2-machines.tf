...
resource "aws_instance" "database" {
  ...
  user_data = <<-HEREDOC
    #!/bin/bash
    sleep 180                    # This sleep 180 is fundamental because if it is not declared, the rest of commands between HEREDOC wonn't be executed correctly
    sudo yum update -y           # as the other aws resources (route tables, groups, gataways ) are not yet completed, up and running.
